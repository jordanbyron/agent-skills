...
Poll the pull or merge request for new review comments and CI or build results.
...
Keep looping until a full pass turns up nothing new: no unanswered comments, no failing checks, no fresh human-comment drafts. If drafts are waiting on user approval, report the run as blocked on the user, not settled, and stop looping instead of polling forever.
